...
export type RunInput = {
  ...
  // Business profile context (from Brand tab, subscription runs only)
  icp?: {
    who: string
    problem: string
    alternatives: string
...
// =============================================================================
// STRATEGY & WEEKLY TASK TYPES (Boost Weekly split pipeline)
// =============================================================================

/**
 * Opus-generated strategic context stored on subscriptions.strategy_context.
 * Refreshed monthly (every 4th week). Foundation for weekly Sonnet task gen.
 */
export type StrategyContext = {
  quarterFocus: {
    primaryObjective: string
    growthLever: string // AARRR stage
    channelStrategy: { primary: string; secondary?: string }
    successMetric: { metric: string; current?: number; target: number }
  ...
  rawStrategy?: string // Full Opus markdown for debugging
  researchSummary?: string // Condensed research findings
  generatedAt: string
  monthNumber: number
}

/**
 * Weekly task output shape stored in runs.structured_output for subscription runs.
 * Generated by Sonnet with WHY/HOW baked in (no separate enrichment step).
 */
export type WeeklyTaskOutput = {
  weekTheme: string
  tasks: Array<{
    title: string
    description: string // success metric
    track: "sprint" | "build"
    why: string
    how: string
    timeEstimate: string
