...
const STAGES = [
  { text: "Reading your subject line", delay: 0, duration: 5 },
  { text: "Evaluating clarity", delay: 5, duration: 5 },
  { text: "Checking urgency signals", delay: 10, duration: 5 },
  { text: "Measuring curiosity gap", delay: 15, duration: 5 },
...
interface Props {
  initialResult: {
    slug: string;
    subjectLine: string;
    output: EmailSubjectAnalysisOutput | null;
    status: ResultStatus;
  };
}

export function EmailSubjectAnalysisResults({ initialResult }: Props) {
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      ...
      try {
        const res = await fetch(`/api/email-subject-scorer/${result.slug}`);
        if (res.ok) {
          const data = await res.json();
          if (data.result.status === "complete" || data.result.status === "failed") {
            stopped = true;
            const input = data.result.input as { subjectLine?: string } | null;
            setResult({
              slug: data.result.slug,
              subjectLine: input?.subjectLine || result.subjectLine,
    ...
    poll();
    return () => { stopped = true; };
  }, [result.slug, result.status, result.subjectLine]);

  // Track results viewed
  useEffect(() => {
    if (result.status === "complete" && !hasTrackedView.current) {
      posthog?.capture("email_subject_scorer_viewed", { slug: result.slug });
        ...
        >
          ...
          <div className="space-y-3 text-sm">
            <div>
              ...
              <span className="text-foreground/60"> — scores your subject line on clarity, urgency, curiosity, and relevance. Plus 3 rewrites.</span>
            </div>
            <div>
      ...
      <div className="max-w-lg mx-auto px-4 py-20 text-center">
        ...
        >
          <p className="text-lg font-semibold text-foreground mb-2">
            We couldn&apos;t score your subject line
          </p>
  ...
  return (
    <div className="max-w-3xl mx-auto px-6 pb-24">
      ...
      <section className="pt-16 pb-12">
        ...
        {/* The subject line being analyzed */}
        <div
      ...
      <section className="mb-12">
        ...
        <p className="text-sm text-foreground/50 mb-6">
          Here&apos;s what your subject line could look like.
        </p>
        ...
        <div className="grid md:grid-cols-[1.2fr_1fr] gap-8 md:gap-12 items-start">
          <div>
            ...
            <h2 className="text-xl lg:text-2xl font-bold text-foreground tracking-tight mb-4">
              Your subject line is one piece. See the full picture.
            </h2>
          ...
          >
            ...
            <a
              href="/upgrade?from=email-subject"
              onClick={() => posthog?.capture("email_subject_cta_clicked", { slug: result.slug })}
